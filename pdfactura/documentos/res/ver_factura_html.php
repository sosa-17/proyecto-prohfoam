<style type="text/css">
<!--
table { vertical-align: top; }
tr    { vertical-align: top; }
td    { vertical-align: top; }
.pumpkin{
	background:#2073d4;
	padding: 4px 4px 4px;
	color:white;
	font-weight:bold;
	font-size:12px;
}
.midnight-blue{
	background:#2073d4;
	padding: 4px 4px 4px;
	color:white;
	font-weight:bold;
	font-size:12px;
}
.silver{
	background:white;
	padding: 3px 4px 3px;
}
.clouds{
	background:#ecf0f1;
	padding: 3px 4px 3px;
}
.border-top{
	border-top: solid 1px #bdc3c7;
	
}
.border-left{
	border-left: solid 1px #bdc3c7;
}
.border-right{
	border-right: solid 1px #bdc3c7;
}
.border-bottom{
	border-bottom: solid 1px #bdc3c7;
}
table.page_footer {width: 100%; border: none; background-color: white; padding: 2mm;border-collapse:collapse; border: none;}
}
-->
</style>
<page backtop="15mm" backbottom="15mm" backleft="15mm" backright="15mm" style="font-size: 12pt; font-family: arial" >
        <page_footer>
       
            
<table cellspacing="0" style="width: 100%;">
        <tr>

            <td  style="width: 25%; color: #444444;">
                <img style="width: 100%;" src="../../img/logofactura.png" alt="Logo"><br>
                
            </td>
			<td style="width: 75%;text-align:right;font-size:24px;color:#2c3e50">
			PEDIDO Nº <?php echo $numero_factura;?>
			</td>
			
        </tr>
    </table>

	
    <table cellspacing="0" style="width: 100%; text-align: left; font-size: 11pt;">
        <tr>
			<td class='pumpkin' style="width:45%;  " >CLIENTE</td>
			<td  style="width:10%; "></td>
			<td class='pumpkin' style="width:45%; ">VENDEDOR</td>
		</tr>
		<tr>
           <td style="width:50%;" >
			<?php 
				$sql_cliente=mysqli_query($con,"select * from usuarios u inner join datospersonales d on u.id=d.usuarios_id where u.id='$id_cliente'");
				$rw_cliente=mysqli_fetch_array($sql_cliente);
					echo $rw_cliente['name'];
				
				echo "<br> Email: ";
				echo $rw_cliente['email'];
				echo "<br> Direccion: ";
				echo $rw_cliente['direccion'];
			?>
			
		   </td>
		   <td  style="width:10%; "></td>
		    <td style="width:50%;" >
			<?php 
				$sql_vendedor=mysqli_query($con,"select * from usuarios u inner join datospersonales d on u.id=d.usuarios_id where u.id='$id_vendedor'");
				$rw_vendedor=mysqli_fetch_array($sql_vendedor);
				echo $rw_vendedor['name'];
				
				echo "<br> Email: ";
				echo $rw_vendedor['email'];
				echo "<br> Direccion: ";
				echo $rw_vendedor['direccion'];
				
				
			?>
			
		   </td>
        </tr>
        
   
    </table>
    <table cellspacing="0" style="width: 100%; text-align: left; font-size: 10pt;">
		<tr>
			<td class='pumpkin' style="width:34%; ">CONDICIONES DE PAGO</td>
			<td class='pumpkin' style="width:33%; text-align:right ">FECHA</td>
		</tr>
		<tr>
			<td style="width:34%; ">
				<?php echo $condicionespedido;?>
			</td>
			<td  style="width:33%; text-align:right"><?php echo date("d-m-Y");?></td>
		</tr>
	</table>
<table cellspacing="0" style="width: 100%; border: solid 0px #7f8c8d; text-align: center; font-size: 10pt;padding:1mm;">
        <tr >
			<th class="pumpkin" style="width: 7% ">CANT.</th>
            <th class="pumpkin" style="width: 55%">DESCRIPCION</th>
            <th class="pumpkin" style="width: 14%;text-align:right">PRECIO UNIT.</th>
            <th class="pumpkin" style="width: 10%;text-align:right">TOTAL</th>
            
        </tr>
   
<?php
$sumador_total=0;
$nums=1;
$sql=mysqli_query($con, "select * from pedidos u inner join detalle_pedido d on u.numero=d.numero_pedido  where u.numero=$numero_factura");
$sql2=mysqli_query($con, "select * from pedidos u inner join detalle_pedido d on u.numero=d.numero_pedido inner join productos p on d.id_producto=p.id where u.numero=$numero_factura");
while ($row=mysqli_fetch_array($sql))
	{
		if ($row2=mysqli_fetch_array($sql2)) {
			$cantidad=$row["cantidad"];
	$DESCRIPCION=$row2["name"];
	$precio=$row2['precio_venta'];
	$total=$cantidad*$precio;
	$sumador_total+=$total;
	if ($nums%2==0){
		$clase="silver";
	} else {
		$clase="clouds";
	}
		}
	
	?>
	
        <tr>
			<td class='<?php echo $clase;?>' style="width: 7%; text-align: center"><?php echo $cantidad; ?></td>
            <td class='<?php echo $clase;?>' style="width: 55%; text-align: left"><?php echo $DESCRIPCION;?></td>
            <td class='<?php echo $clase;?>' style="width: 14%; text-align: right"><?php echo $precio;?></td>
            <td class='<?php echo $clase;?>' style="width: 10%; text-align: right"><?php echo $total;?></td>
            
        </tr>
    
	<?php 
	//Insert en la tabla detalle_cotizacion
	
	$nums++;

	}
	$iva=$sumador_total*(0.15); 
	$total_pagar=$sumador_total+$iva;
?>
	</table>
<table cellspacing="0" style="width: 100%; border: solid 0px black; background: white; font-size: 11pt;padding:1mm;">
        <tr>
			<th style="width: 50%; text-align: right;"></th>
            <th style="width: 37%; text-align: right;">SUBTOTAL Lps.</th>
            <th style="width: 13%; text-align: right;"><?php echo number_format($sumador_total,2);?></th>
        </tr>
		<tr>
			<th class='pumpkin' style="width: 50%; text-align: center;">Comentarios o instruciones especiales</th>
            <th style="width: 37%; text-align: right;">IVA  Lps.</th>
            <th style="width: 13%; text-align: right;"><?php echo number_format($iva,2);?></th>
        </tr>
		<tr>
			<td class='border-left border-bottom border-right' style="width: 50%;"><?php echo $comentariopedido;?></td>
            <th  style="width: 37%; text-align: right;">TOTAL Lps. </th>
            <th style="width: 13%; text-align: right;"><?php echo number_format($total_pagar,2);?></th>
        </tr>
    </table>



    </page_footer>
	<?php include("encabezado_factura.php");?>
    <br>
   
    
       
	
	
	<br>
	<div style="font-size:11pt;text-align:center;font-weight:bold">Gracias por su compra!</div>
	
	
	  

</page>

